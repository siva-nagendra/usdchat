...
def process_docs_and_create_csv(
    dir_path, embeddings_csv_path, chunks_csv_path, chunk_size=1000
):
...
def summary_agent(prompt):
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You give a brief summary of given text. \
...
def query_agent(prompt):
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You answer a user's question, given some text as context to help\
...
def doc_agent(prompt):
    ...
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You take a user request about documents in a folder. \
    ...
    if query == "summarize":
        ...
        # return summary
        return [
            {"role": "user", "content": prompt + " " + summary_chunk},
